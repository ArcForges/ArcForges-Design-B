...
3. Any number of workers may execute ready tasks concurrently. A task is ready when its owning repository has completed adoption and its start prerequisites are satisfied: a contract, artifact or design prerequisite by the producer's delivered outcome, a release prerequisite only by completion. It is claimed through an atomic claim branch with a lease in the Plan repository; interrupted work resumes from its retained worktree and pull request, and re-claiming a released task or taking over an expired lease is an atomic append to the same claim.
